...
export default function Post() {
  ...
  const getpost = async () => {
    try{
      let respost = await fetch(`http://localhost:4000/api/v1/post/` + path, {
        method: "GET",
        headers: {
  ...
  const handledelete = async () => {
    try {
      await axios.delete(`http://localhost:4000/api/v1/post/${post._id}`,
        { data: { name: user.name } });
  ...
  const handleEdit=async()=>{
    try{
      await axios.put(`http://localhost:4000/api/v1/post/${post._id}`,
